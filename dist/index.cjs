...
function getMetadataClient() {
  if (!metadataClient || metadataClient.network !== activeNetwork) {
    metadataClient = {
      client: new import_jsonRpc.SuiJsonRpcClient({ url: (0, import_jsonRpc.getJsonRpcFullnodeUrl)(activeNetwork), network: activeNetwork }),
...
var DeepBookPool = class {
  ...
  constructor(network = "testnet", address) {
    ...
    const pools = network === "testnet" ? import_deepbook_v32.testnetPools : import_deepbook_v32.mainnetPools;
    this.client = new import_deepbook_v3.DeepBookClient({
  ...
  async getQuote(tokenIn, tokenOut, amountIn) {
    try {
      ...
      const pool = this.network === "testnet" ? import_deepbook_v32.testnetPools[poolKey] : import_deepbook_v32.mainnetPools[poolKey];
      if (!pool) return null;
...
var CetusPool = class {
  constructor(network, senderAddress) {
    this.network = network;
    this.sdk = (0, import_cetus_sui_clmm_sdk.initCetusSDK)({
      network,
...
var AftermathPool = class {
  constructor(network) {
    this.network = network;
    this.sdk = new import_aftermath_ts_sdk.Aftermath(NETWORK_MAP[network]);
  }
  network;
  sdk;
  router = null;
...
var TurbosPool = class {
  ...
  constructor(network) {
    this.network = network;
    this.sdk = new import_turbos_clmm_sdk.TurbosSdk(
      network === "mainnet" ? import_turbos_clmm_sdk.Network.mainnet : import_turbos_clmm_sdk.Network.testnet
    );
  }
  network;
  sdk;
...
var FlowXPool = class {
  ...
  constructor(network) {
    this.network = network;
    this.quoter = new import_sdk.AggregatorQuoter(network);
  }
  async getQuote(tokenIn, tokenOut, amountIn) {
    try {
  ...
  async buildSwapTransaction(tokenIn, tokenOut, amountIn, slippage, senderAddress) {
    ...
    try {
      const trade = new import_sdk.TradeBuilder(this.network, routes).sender(senderAddress).slippage(slippage * 1e6).commission(null).build();
      return null;
    } catch {
      return null;
    }
...
// src/pools/hop.ts
var import_sdk2 = require("@hop.ag/sdk");
...
var HopPool = class {
  ...
  constructor(_network) {
    this.sdk = new import_sdk2.HopApi({
...
var SuiTransactions = __toESM(require("@mysten/sui/transactions"), 1);
...
var SevenKProtocolPool = class {
  ...
  async addSwapToTransaction(tx, tokenIn, tokenOut, amountIn, senderAddress, slippageBps = 100, coinInOverride) {
    ...
    const coinIn = coinInOverride ?? (0, import_transactions.coinWithBalance)({
      balance: amountIn,
      type: tokenIn.type
    });
...
var PTBBuilder = class {
  ...
  async buildCetusStep(tx, step, senderAddress, slippage, inputCoin) {
    ...
    if (inputCoin) {
      ...
      const { txRes } = import_cetus_sui_clmm_sdk2.TransactionUtil.buildSwapTransactionWithoutTransferCoinArgs(
        sdk,
        tx,
        params,
        sdkOptions,
      ...
      const { tx: builtTx, txRes } = import_cetus_sui_clmm_sdk2.TransactionUtil.buildSwapTransactionWithoutTransferCoinArgs(
        sdk,
        tx,
        params,
        sdkOptions,
